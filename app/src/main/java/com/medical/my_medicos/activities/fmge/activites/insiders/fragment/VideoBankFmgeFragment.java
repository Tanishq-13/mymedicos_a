...
import com.medical.my_medicos.databinding.FragmentVideoBankFmgeBinding;
...
public class VideoBankFmgeFragment extends Fragment {
    ...
    public static VideoBankFmgeFragment newInstance(int catId, String title) {
        VideoBankFmgeFragment fragment = new VideoBankFmgeFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentVideoBankFmgeBinding.inflate(inflater, container, false);
        View view = binding.getRoot();

        String title = getArguments().getString("title", "");

        if (getActivity() instanceof SpecialityFMGEInsiderActivity) {
            ((SpecialityFMGEInsiderActivity) getActivity()).setToolbarTitle(title);
        }

        videosforpg = new ArrayList<>();
        videosAdapter = new VideoFMGEAdapter(requireContext(), videosforpg);
